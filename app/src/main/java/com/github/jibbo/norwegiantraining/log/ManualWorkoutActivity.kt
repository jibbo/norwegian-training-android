        ...
        setContent {
            LaunchedEffect(Unit) {
                viewModel.savedSessionEvent.collect { session ->
                    setResult(RESULT_OK, Intent().putExtra(EXTRA_SAVED_DATE, session.date.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().toString()))
                    finish()
                }
            }
...
@Composable
internal fun ManualWorkoutScreen(viewModel: ManualWorkoutViewModel, initialDate: LocalDate, onBack: () -> Unit) {
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            FlowRow(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ManualWorkoutType.values().forEach { type ->
                    Button(
                        ...
                        modifier = Modifier.fillMaxWidth(0.48f).testTag("manual_workout_type_${type.name}"),
                        contentPadding = PaddingValues(horizontal = 8.dp),
                        colors = ButtonDefaults.buttonColors(if (draft.type == type) Primary else Gray, if (draft.type == type) Black else White),
                    ) { Text(manualWorkoutTypeLabel(type), textAlign = TextAlign.Center, maxLines = 2) }
                }
            }
            state.fieldErrors[ManualWorkoutField.TYPE]?.let { ManualWorkoutErrorText(R.string.manual_workout_error_type_required, "manual_workout_error_type") }
            Text(R.string.manual_workout_date.localizable(), style = Typography.bodyMedium)
            Text(draft.date.toString(), color = White, modifier = Modifier.fillMaxWidth().clickable { showDatePicker = true }.padding(vertical = 12.dp).testTag("manual_workout_date"))
            ...
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                DurationField(draft.hours, R.string.manual_workout_duration_hours.localizable(), "manual_workout_hours", viewModel::updateHours)
                Text(":", style = Typography.titleLarge)
                DurationField(draft.minutes, R.string.manual_workout_duration_minutes.localizable(), "manual_workout_minutes", viewModel::updateMinutes)
            }
            state.fieldErrors.values.distinct().forEach { ManualWorkoutErrorText(manualWorkoutErrorResource(it)) }
            if (state.persistenceError) ManualWorkoutErrorText(R.string.manual_workout_error_save_failed)
            Spacer(Modifier.height(12.dp))
            Button(onClick = { viewModel.submit(translatedName) }, enabled = !state.isSaving, modifier = Modifier.fillMaxWidth().testTag("manual_workout_save")) {
                if (state.isSaving) CircularProgressIndicator(Modifier.size(18.dp)) else Text(R.string.manual_workout_save.localizable())
            }
        }
    }
    if (showDatePicker) {
        ...
        val pickerState = rememberDatePickerState(
            initialSelectedDateMillis = draft.date.atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli(),
        ...
        DatePickerDialog(onDismissRequest = { showDatePicker = false }, confirmButton = {
            TextButton(onClick = { pickerState.selectedDateMillis?.let { viewModel.updateDate(Instant.ofEpochMilli(it).atZone(ZoneId.systemDefault()).toLocalDate()) }; showDatePicker = false }) { Text(R.string.ok.localizable()) }
        }, dismissButton = { TextButton(onClick = { showDatePicker = false }) { Text(R.string.close.localizable()) } }) { DatePicker(state = pickerState, showModeToggle = false) }
    }
}

@Composable private fun DurationField(value: String, label: String, tag: String, onValueChange: (String) -> Unit) = OutlinedTextField(value, { onValueChange(it.filter(Char::isDigit).take(2)) }, label = { Text(label) }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true, modifier = Modifier.testTag(tag).fillMaxWidth(0.42f))
@Composable private fun ManualWorkoutErrorText(resource: Int, tag: String? = null) = Text(resource.localizable(), color = Primary, style = Typography.bodySmall, modifier = if (tag == null) Modifier else Modifier.testTag(tag))
...
@Composable private fun manualWorkoutTypeLabel(type: ManualWorkoutType) = when (type) {
